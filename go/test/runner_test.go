...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("HOMEPAGESCREENSHOT_TEST_LIVE") == "TRUE" ||
		os.Getenv("HOMEPAGESCREENSHOT_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("HOMEPAGESCREENSHOT_TEST_EXPLAIN"); explain != "" {
