...
func TestGetScreenshotByDomainAndDateDirect(t *testing.T) {
	t.Run("direct-load-get_screenshot_by_domain_and_date", func(t *testing.T) {
		setup := get_screenshot_by_domain_and_dateDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-get_screenshot_by_domain_and_date", _mode); _shouldSkip {
			if _reason == "" {
...
type get_screenshot_by_domain_and_dateDirectSetupResult struct {
...
func get_screenshot_by_domain_and_dateDirectSetup(mockres any) *get_screenshot_by_domain_and_dateDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		return &get_screenshot_by_domain_and_dateDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &get_screenshot_by_domain_and_dateDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
