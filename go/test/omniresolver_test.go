...
import (
	...
	omni "github.com/voxgig-sdk/homepage-screenshot-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.HomepageScreenshotError); is {
				out := map[string]any{
					"name":    "HomepageScreenshotError",
...
func omniCtx(arg any, client *sdk.HomepageScreenshotSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.HomepageScreenshotSDK); is {
			client = live
