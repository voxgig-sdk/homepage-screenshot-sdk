...
[MCP](https://modelcontextprotocol.io) server exposing the HomepageScreenshot SDK as
two agent tools — `homepage-screenshot_list` and `homepage-screenshot_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/homepage-screenshot-mcp)
make build

# 2. Provide credentials via the environment
export HOMEPAGE_SCREENSHOT_APIKEY=sk_live_xxx
...
```jsonc
// homepage-screenshot_list: first page of records
{ "entity": "get_screenshot_by_domain" }
{ "entity": "get_screenshot_by_domain", "query": { } }

// homepage-screenshot_load: one record by id
{ "entity": "get_screenshot_by_domain", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/homepage-screenshot-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user homepage-screenshot \
     -- "$PWD"/dist/*/homepage-screenshot-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `homepage-screenshot_list` and `homepage-screenshot_load` tools now appear
   in new sessions. Ask the agent to *"list get_screenshot_by_domain using homepage-screenshot"*
   and it calls `homepage-screenshot_list` with `{"entity":"get_screenshot_by_domain"}`.
...
```sh
export HOMEPAGE_SCREENSHOT_APIKEY=sk_live_xxx            # API key
export HOMEPAGE_SCREENSHOT_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `homepage-screenshot_list` tool
...
### Call the `homepage-screenshot_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `homepage-screenshot_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `homepage-screenshot_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `HOMEPAGE_SCREENSHOT_APIKEY` | API key sent with every request. |
| `HOMEPAGE_SCREENSHOT_BASE` | Optional override of the API base URL. |
