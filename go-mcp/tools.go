...
type Args struct {
	Entity string         `json:"entity" jsonschema:"get_screenshot_by_domain | get_screenshot_by_domain_and_date"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.HomepageScreenshotSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "homepage-screenshot_list",
		Description: "List records from HomepageScreenshot. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "homepage-screenshot_load",
		Description: "Load a single record from HomepageScreenshot. " +
...
func runOp(client *sdk.HomepageScreenshotSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.HomepageScreenshotSDK, name string) (sdk.HomepageScreenshotEntity, error) {
	switch strings.ToLower(name) {
	case "get_screenshot_by_domain":
		return client.GetScreenshotByDomain(nil), nil
	case "get_screenshot_by_domain_and_date":
		return client.GetScreenshotByDomainAndDate(nil), nil
