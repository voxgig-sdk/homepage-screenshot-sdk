// Command HomepageScreenshot-cli is a CLI + REPL for the HomepageScreenshot SDK,
...
const prompt = "homepage-screenshot"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_screenshot_by_domain get_screenshot_by_domain_and_date"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      HOMEPAGE_SCREENSHOT_APIKEY (api key), HOMEPAGE_SCREENSHOT_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: HOMEPAGE_SCREENSHOT_APIKEY carries the API key and
	// HOMEPAGE_SCREENSHOT_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("HOMEPAGE_SCREENSHOT_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("HOMEPAGE_SCREENSHOT_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewHomepageScreenshotSDK(opts)
