...
boru-driven command-line client **and** interactive REPL for the HomepageScreenshot
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/homepage-screenshot-cli)
make build

# 2. See usage (words, entities, env vars)
./homepage-screenshot-cli --help

# 3. Provide credentials once, via the environment
export HOMEPAGE_SCREENSHOT_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./homepage-screenshot-cli load 1 get_screenshot_by_domain            # {id:1} shorthand
./homepage-screenshot-cli load '{id:1}' get_screenshot_by_domain       # explicit match map

# 5. Override the API base URL for a single call
HOMEPAGE_SCREENSHOT_BASE=https://api.example.com ./homepage-screenshot-cli load 1 get_screenshot_by_domain

# 6. No arguments -> interactive REPL
./homepage-screenshot-cli
homepage-screenshot> load 1 get_screenshot_by_domain
homepage-screenshot> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/homepage-screenshot-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/homepage-screenshot-cli load 1 get_screenshot_by_domain
   ```
...
```sh
./homepage-screenshot-cli load 1 get_screenshot_by_domain          # scalar shorthand for {id:1}
./homepage-screenshot-cli load '{id:1}' get_screenshot_by_domain     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `homepage-screenshot>`). Each line is
evaluated as its own boru expression:

```text
$ ./homepage-screenshot-cli
homepage-screenshot> load 1 get_screenshot_by_domain
homepage-screenshot> /help
homepage-screenshot> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_screenshot_by_domain`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/homepage-screenshot-cli`. |
...
(`./homepage-screenshot-cli <expr>`) and interactively (the REPL), and expressions compose the
