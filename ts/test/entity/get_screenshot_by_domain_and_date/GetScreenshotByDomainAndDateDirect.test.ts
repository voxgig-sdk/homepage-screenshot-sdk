...
describe('GetScreenshotByDomainAndDateDirect', async () => {
  ...
  test('direct-load-get_screenshot_by_domain_and_date', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-get_screenshot_by_domain_and_date', setup.live)) return
