...
//     (\`new HomepageScreenshotSDK(...)\` / \`HomepageScreenshotSDK.test(...)\` become
//     \`HomepageScreenshotSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'HomepageScreenshotSDK'
...
const TEST_SEED = {"entity":{"get_screenshot_by_domain":{"test01":{"id":"test01"}},"get_screenshot_by_domain_and_date":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new HomepageScreenshotSDK(...)\` and any
// \`HomepageScreenshotSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+HomepageScreenshotSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/HomepageScreenshotSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
