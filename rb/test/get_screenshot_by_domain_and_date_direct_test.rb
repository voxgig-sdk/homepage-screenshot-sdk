...
class GetScreenshotByDomainAndDateDirectTest < Minitest::Test
  def test_direct_load_get_screenshot_by_domain_and_date
    setup = get_screenshot_by_domain_and_date_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-get_screenshot_by_domain_and_date", setup[:live] ? "live" : "unit")
    if _should_skip
...
def get_screenshot_by_domain_and_date_direct_setup(mockres)
