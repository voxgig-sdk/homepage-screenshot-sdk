...
class GetScreenshotByDomainEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set HOMEPAGESCREENSHOT_TEST_GET_SCREENSHOT_BY_DOMAIN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    get_screenshot_by_domain_ref01_data_dt0_loaded, err = get_screenshot_by_domain_ref01_ent.load(get_screenshot_by_domain_ref01_match_dt0, nil)
    assert_nil err
    assert !get_screenshot_by_domain_ref01_data_dt0_loaded.nil?
...
def get_screenshot_by_domain_basic_setup(extra)
  ...
  entid_env_raw = ENV["HOMEPAGESCREENSHOT_TEST_GET_SCREENSHOT_BY_DOMAIN_ENTID"]
