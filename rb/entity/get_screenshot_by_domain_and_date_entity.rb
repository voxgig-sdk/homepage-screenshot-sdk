...
class GetScreenshotByDomainAndDateEntity
  def initialize(client, entopts = nil)
    ...
    @_name = "get_screenshot_by_domain_and_date"
    @_client = client
  ...
  def make
    opts = @_entopts.dup
    GetScreenshotByDomainAndDateEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = HomepageScreenshotHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetScreenshotByDomainAndDate, Hash] the current GetScreenshotByDomainAndDate data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = HomepageScreenshotHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetScreenshotByDomainAndDate fields)
  def match_get
  ...
  #
  # @param reqmatch [GetScreenshotByDomainAndDateLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.GetScreenshotByDomainAndDate.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [GetScreenshotByDomainAndDate, Hash] the loaded GetScreenshotByDomainAndDate; raises HomepageScreenshotError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = HomepageScreenshotHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
