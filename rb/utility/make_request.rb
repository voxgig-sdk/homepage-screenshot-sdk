...
module HomepageScreenshotUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = HomepageScreenshotResponse.new({})
    ...
    if fetch_err
      ...
      response = HomepageScreenshotResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = HomepageScreenshotResponse.new(fetched)
    else
