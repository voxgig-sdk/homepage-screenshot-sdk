...
require_relative 'HomepageScreenshot_types'
...
class HomepageScreenshotSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = HomepageScreenshotHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = HomepageScreenshotHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, HomepageScreenshotFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    HomepageScreenshotUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = HomepageScreenshotHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = HomepageScreenshotHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = HomepageScreenshotHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => HomepageScreenshotError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue HomepageScreenshotError => err
    ...
    if fetched.is_a?(Hash)
      status = HomepageScreenshotHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = HomepageScreenshotError.new(
        "graphql_error", "HomepageScreenshotSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = HomepageScreenshotSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
