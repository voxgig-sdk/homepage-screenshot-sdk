# HomepageScreenshot Ruby SDK



The Ruby SDK for the HomepageScreenshot API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.GetScreenshotByDomain` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = HomepageScreenshotSDK.test(testopts, sdkopts)
```
...
| `GetScreenshotByDomain` | `(data) -> GetScreenshotByDomainEntity` | Create a GetScreenshotByDomain entity instance. |
| `GetScreenshotByDomainAndDate` | `(data) -> GetScreenshotByDomainAndDateEntity` | Create a GetScreenshotByDomainAndDate entity instance. |
...
raise a `HomepageScreenshotError` (a `StandardError` subclass), so wrap
...
```
rb/
├── HomepageScreenshot_sdk.rb       -- Main SDK module
...
The main module (`HomepageScreenshot_sdk`) exports the SDK class
...
# getscreenshotbydomain.data_get now returns the getscreenshotbydomain data from the last load
# getscreenshotbydomain.match_get returns the last match criteria
```
