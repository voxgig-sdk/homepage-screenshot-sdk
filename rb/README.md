...
The Ruby SDK for the HomepageScreenshot API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.GetScreenshotByDomain(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.HomepageScreenshot(nil).load(
  { "id" => "test01" }, nil
)
