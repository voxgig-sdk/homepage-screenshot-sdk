...
//    only - a typed HomepageScreenshotContext there would make every ctx
...
final class HomepageScreenshotOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new HomepageScreenshotContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new HomepageScreenshotSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new HomepageScreenshotResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new HomepageScreenshotError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new HomepageScreenshotResponse($clean['response']);
    ...
    public static function err_from_map($m): ?HomepageScreenshotError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new HomepageScreenshotError(is_string($code) ? $code : '', $msg);
    }
