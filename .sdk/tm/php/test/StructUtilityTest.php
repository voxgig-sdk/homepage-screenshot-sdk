...
// HomepageScreenshot SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = HomepageScreenshotOmni::makeStructRunner(
    ...
    {
        return HomepageScreenshotOmni::objmodel(HomepageScreenshotOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return HomepageScreenshotOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return HomepageScreenshotOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(HomepageScreenshotOmni::structfix($now, Struct::undef()))
                        !== json_encode(HomepageScreenshotOmni::structfix($val, Struct::undef()))) {
