...
// HomepageScreenshot SDK retry feature
...
class HomepageScreenshotRetryFeature extends HomepageScreenshotBaseFeature
{
    ...
    public function init(HomepageScreenshotContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (HomepageScreenshotContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(HomepageScreenshotContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(HomepageScreenshotContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
