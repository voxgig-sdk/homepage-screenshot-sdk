...
// HomepageScreenshot SDK GetScreenshotByDomainAndDate entity
...
class GetScreenshotByDomainAndDateEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "get_screenshot_by_domain_and_date";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new GetScreenshotByDomainAndDateEntity($this->_client, $opts);
    }

    /**
     * @param GetScreenshotByDomainAndDate|array $args GetScreenshotByDomainAndDate data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = HomepageScreenshotHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetScreenshotByDomainAndDate|array The current GetScreenshotByDomainAndDate data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetScreenshotByDomainAndDate fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = HomepageScreenshotHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetScreenshotByDomainAndDate fields).
     */
    ...
    /**
     * Load a single GetScreenshotByDomainAndDate.
     *
     * @param GetScreenshotByDomainAndDateLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed GetScreenshotByDomainAndDateLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetScreenshotByDomainAndDate|array The loaded GetScreenshotByDomainAndDate as an assoc-array at the
     *   SDK boundary; throws HomepageScreenshotError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = HomepageScreenshotHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
