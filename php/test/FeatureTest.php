...
// HomepageScreenshot SDK feature test
//
...
class FtCtrl extends HomepageScreenshotControl
{
...
class FtClient
{
    ...
    private HomepageScreenshotUtility $utility;

    public function __construct(HomepageScreenshotUtility $utility, array $options)
    {
    ...
    public function get_utility(): HomepageScreenshotUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (HomepageScreenshotContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public HomepageScreenshotContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (HomepageScreenshotContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = HomepageScreenshotConfig::make_feature($name);
    ...
    private static function buildUrl(HomepageScreenshotSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof HomepageScreenshotError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new HomepageScreenshotResponse($fetched);
        }
    ...
    private function fail(HomepageScreenshotContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (HomepageScreenshotContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
