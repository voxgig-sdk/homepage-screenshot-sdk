...
// HomepageScreenshot SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class HomepageScreenshotTestFeature extends HomepageScreenshotBaseFeature
{
    ...
    public function init(HomepageScreenshotContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (HomepageScreenshotContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = HomepageScreenshotParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (HomepageScreenshotContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(HomepageScreenshotContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = HomepageScreenshotParam::call($ctx, $k);
