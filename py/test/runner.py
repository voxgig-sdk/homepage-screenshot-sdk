# HomepageScreenshot SDK test runner
...
class HomepageScreenshotTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            HomepageScreenshotTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = HomepageScreenshotTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = HomepageScreenshotTestRunner.getenv("HOMEPAGE_SCREENSHOT_TEST_LIVE")
        override = HomepageScreenshotTestRunner.getenv("HOMEPAGE_SCREENSHOT_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = HomepageScreenshotTestRunner.getenv(key)
        ...
        explain = HomepageScreenshotTestRunner.getenv("HOMEPAGE_SCREENSHOT_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["HOMEPAGE_SCREENSHOT_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if HomepageScreenshotTestRunner._test_control is not None:
            return HomepageScreenshotTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                HomepageScreenshotTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            HomepageScreenshotTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = HomepageScreenshotTestRunner.load_test_control()
...
def load_env_local():
    HomepageScreenshotTestRunner.load_env_local()


def env_override(m):
    return HomepageScreenshotTestRunner.env_override(m)


def entity_data(v):
    return HomepageScreenshotTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return HomepageScreenshotTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return HomepageScreenshotTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return HomepageScreenshotTestRunner.load_test_control()


def live_client_options():
    return HomepageScreenshotTestRunner.live_client_options()


def live_delay_ms():
    return HomepageScreenshotTestRunner.live_delay_ms()
