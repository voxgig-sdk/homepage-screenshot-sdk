...
class TestGetScreenshotByDomainEntity:

    def test_should_create_instance(self):
        testsdk = HomepageScreenshotSDK.test(None, None)
        ent = testsdk.GetScreenshotByDomain(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _get_screenshot_by_domain_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "get_screenshot_by_domain." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HOMEPAGE_SCREENSHOT_TEST_GET_SCREENSHOT_BY_DOMAIN_ENTID JSON to run live")
        client = setup["client"]
        ...
        get_screenshot_by_domain_ref01_data = None
        if len(get_screenshot_by_domain_ref01_data_raw) > 0:
        ...
        get_screenshot_by_domain_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(get_screenshot_by_domain_ref01_data_dt0_loaded))
        assert get_screenshot_by_domain_ref01_data_dt0_load_result is not None
        assert get_screenshot_by_domain_ref01_data_dt0_load_result["id"] == get_screenshot_by_domain_ref01_data["id"]



def _get_screenshot_by_domain_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/get_screenshot_by_domain/GetScreenshotByDomainTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("HOMEPAGE_SCREENSHOT_TEST_GET_SCREENSHOT_BY_DOMAIN_ENTID"))
