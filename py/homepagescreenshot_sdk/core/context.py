...
from homepagescreenshot_sdk.core.helpers import get_ctx_prop, to_map
...
class HomepageScreenshotContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = HomepageScreenshotControl()
        ...
        if isinstance(sp, HomepageScreenshotSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, HomepageScreenshotResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, HomepageScreenshotResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return HomepageScreenshotOperation({})
    ...
    def make_error(self, code, msg):
        return HomepageScreenshotError(code, msg, self)
