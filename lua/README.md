...
The Lua SDK for the HomepageScreenshot API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:GetScreenshotByDomain(nil):load({ id = "example_id" }, nil)
...
local result, err = client:HomepageScreenshot(nil):load(
  { id = "test01" }, nil
)
