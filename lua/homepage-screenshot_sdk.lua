...
require("homepage-screenshot_types")
...
local HomepageScreenshotSDK = {}
HomepageScreenshotSDK.__index = HomepageScreenshotSDK
...
HomepageScreenshotSDK._make_feature = _make_feature


function HomepageScreenshotSDK.new(options)
  local self = setmetatable({}, HomepageScreenshotSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function HomepageScreenshotSDK:options_map()
...
function HomepageScreenshotSDK:get_utility()
  return Utility.copy(self._utility)
end


function HomepageScreenshotSDK:get_root_ctx()
  return self._rootctx
end


function HomepageScreenshotSDK:prepare(fetchargs)
...
function HomepageScreenshotSDK:direct(fetchargs)
...
function HomepageScreenshotSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function HomepageScreenshotSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "HomepageScreenshotSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function HomepageScreenshotSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function HomepageScreenshotSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "HomepageScreenshotSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function HomepageScreenshotSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = HomepageScreenshotSDK.new(sdkopts)
  sdk.mode = "test"
