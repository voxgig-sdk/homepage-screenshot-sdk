...
```lua
local result, err = client:GetScreenshotByDomain(nil):load({ id = "get_screenshot_by_domain_id" }, nil)
```
...
```lua
local result, err = client:GetScreenshotByDomainAndDate(nil):load({ id = "get_screenshot_by_domain_and_date_id" }, nil)
```
